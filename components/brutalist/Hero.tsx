...
import { heroConfig, navigationConfig } from '@/data/brutalConfig';

const AsciiCanvas = dynamic(() => import('./AsciiCanvas'), {
  ssr: false,
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="brutalist-hero-nav-links" style={{ display: 'flex', gap: '32px' }}>
            {navigationConfig.links.map((item, index) => (
              <div key={`${item.label}-${item.href}`} style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
                ...
                {index < navigationConfig.links.length - 1 && (
                  <span className="nav-dot" style={{ color: 'rgba(255,255,255,0.3)', fontSize: '12px' }}>·</span>
                )}
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div
            className="brutalist-hero-notes"
            style={{
              position: 'relative',
              marginTop: '54px',
              width: '100%',
