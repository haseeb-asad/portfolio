...
import { facilitiesConfig, navigationConfig } from '@/data/brutalConfig';
import type { GetStaticPaths, GetStaticProps } from 'next';

export const getStaticPaths: GetStaticPaths = async () => {
  const paths = facilitiesConfig.items.map((item) => ({
...
export default function ProjectDetail({ slug }: { slug: string }) {
  ...
  return (
    <>
      <Head>
        <title>{`${facility.name} - Haseeb Asad`}</title>
        <meta name="description" content={facility.article.paragraphs[0]} />
      </Head>
      <div
        style={{
          minHeight: '100vh',
